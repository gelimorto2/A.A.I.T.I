...
# A.A.I.T.I Verbose Demo Script for macOS
# Version: 2.0.0
# This script provides detailed feedback and progress tracking for the demo on macOS
...
print_header() {
    ...
    cat << "EOF"
╔══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╗
║                          🍎 A.A.I.T.I v2.0.0 - VERBOSE DEMO LAUNCHER (macOS)                                        ║
...
main() {
    ...
    log_message "Starting A.A.I.T.I verbose demo on macOS"
    ...
    echo ""
    echo -e "${CYAN}macOS Version: $(get_macos_version)${NC}"
    echo -e "${CYAN}Log file: $LOG_FILE${NC}"
    echo -e "${CYAN}Project root: $PROJECT_ROOT${NC}"
    echo ""
    
    # Step 1: System Requirements Check
    print_step "1" "8" "Checking macOS System Requirements & Dependencies"
    start_timer
    
    print_substep "Checking macOS version compatibility..."
    MACOS_VERSION=$(get_macos_version)
    MACOS_MAJOR=$(echo $MACOS_VERSION | cut -d. -f1)
    if [ "$MACOS_MAJOR" -ge 11 ]; then
        print_success "macOS $MACOS_VERSION is supported (requires 11.0+)"
    else
        print_warning "macOS $MACOS_VERSION may have compatibility issues (11.0+ recommended)"
    fi
    log_message "macOS version: $MACOS_VERSION"
    
    print_substep "Checking Docker Desktop installation..."
    if ! command -v docker &> /dev/null; then
        print_error "Docker Desktop is required for the demo"
        echo -e "   ${WHITE}Install Docker Desktop for Mac:${NC}"
        echo -e "     Intel Macs: https://docs.docker.com/desktop/install/mac-install/"
        echo -e "     Apple Silicon: https://docs.docker.com/desktop/install/mac-install/"
    ...
    print_substep "Checking Docker Desktop service status..."
    if ! docker info &> /dev/null; then
        print_error "Docker Desktop is not running"
        echo -e "   ${WHITE}Start Docker Desktop:${NC}"
        echo -e "     1. Open Docker Desktop from Applications"
        echo -e "     2. Wait for Docker Desktop to start (whale icon in menu bar)"
        echo -e "     3. Re-run this script"
        if check_docker_desktop; then
            print_info "Docker Desktop app is running but service is not ready"
            print_info "Please wait for Docker Desktop to fully start and try again"
        else
            print_info "Docker Desktop app is not running"
    ...
    print_success "Docker Desktop service is running and accessible"
    ...
    print_substep "Checking macOS system resources..."
    # Check available memory
    AVAILABLE_MEM=$(vm_stat | perl -ne '/page size of (\d+)/ and $size=$1; /Pages\s+free:\s+(\d+)/ and printf("%.1fGB", $1 * $size / 1024**3)')
    ...
    if [ "$(uname -m)" = "arm64" ]; then
        print_info "Apple Silicon Mac detected - Docker will use native ARM64 containers when available"
    else
        print_info "Intel Mac detected - Docker will use x86_64 containers"
    fi
    ...
    if docker info | grep -q "Total Memory"; then
        DOCKER_MEM=$(docker info | grep "Total Memory" | awk '{print $3 $4}')
        print_info "Docker Desktop memory allocation: $DOCKER_MEM"
    fi
    ...
    print_info "This may take 2-5 minutes on first run (longer on Apple Silicon)"
    ...
    print_info "This may take 3-7 minutes on first run (longer on Apple Silicon)"
    ...
    echo -e "   ${CYAN}• Full reset: $COMPOSE_CMD -f docker-compose.demo.yml down -v && ./scripts/macos/demo-verbose.sh${NC}"
    ...
    print_info "Using macOS 'open' command to launch browser..."
    sleep 3
    ...
    if command -v osascript &> /dev/null; then
        osascript -e 'display notification "A.A.I.T.I Demo is ready!" with title "Demo Launcher" sound name "Glass"' 2>/dev/null || true
    fi
...
EOF
    ...
    echo -e "${WHITE}🍎 macOS Version: ${CYAN}$MACOS_VERSION${NC}"
    echo ""
    ...
    echo ""
    echo -e "${PURPLE}💡 Tip: Keep Docker Desktop running for optimal performance${NC}"
    
    log_message "Demo launched successfully in ${total_time}s on macOS $MACOS_VERSION"
}
